...
from aerial_training_utils import parallel_make_dataset
...
# min(multiprocessing.cpu_count(),40) # Change to speed up extraction in case multicore architecture is available
MAX_NB_JOBS = 1
...
def reproject_tile(ori_tile):
    ...
    subprocess.run(['gdalwarp', '-t_srs', 'EPSG:3035', '-r', 'bilinear',
                   '-co', 'BIGTIFF=YES', '-co', 'COMPRESS=LZW', ori_tile, reproj_tile])
    # os.system(
    #    "gdalwarp -t_srs EPSG:3035 -r bilinear -co BIGTIFF=YES -co COMPRESS=LZW %s %s"
    #    % (ori_tile, reproj_tile))  # check this line
    return reproj_tile
...
def subextract_from_aerial_tile(tile_fnames, insee_cells_to_extract):
    ...
    if len(tile_fnames.split(";")) == 1:
        ...
        curr_sat_dir = tile_fnames.split("/")[9]
        ...
        for name in tile_fnames.split(';'):
            tiff_tile += ';'+reproject_tile(name)

        tiff_tile = tiff_tile[1:]
        merged_tile = merge_coll_tiles(tiff_tile)
        # reproj_tile = reproject_tile(merged_tile)
        aerial_data = rasterio.open(merged_tile)
...
def main():
    ...
    geo_df_data = gdf.from_file(
        CENSUS_DIR + 'Filosofi2015_carreaux_200m_metropole.shp')
    ...
    geo_census_ua = gpd.sjoin(
        geo_df_data, geo_ua_bound).drop("index_right", axis=1)
    extract_ua = geo_census_ua[["idINSPIRE", "city"]]
    extract_ua["FUA_NAME"] = [x[0:1].upper() + x[1:].lower()
                              for x in extract_ua.city]
    extract_ua[["idINSPIRE", "FUA_NAME"]].to_csv(
        AERIAL_DIR + "city_assoc.csv", index=False)
    ...
    df_insee2aerial = gpd.sjoin(
        geo_census_ua, geo_aerial_data, op="intersects")
